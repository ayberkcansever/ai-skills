> **Supporting files:** this skill uses `template.html`, `lab-template.html`, `notes-template.html`, and `scripts/build_index.py` shipped in the Cursor variant folder (`learning/cursor/learn/`). Copy them next to wherever you keep this command's assets, or install the Cursor variant alongside and keep the default `~/.cursor/skills/learn/` paths referenced in the text.
