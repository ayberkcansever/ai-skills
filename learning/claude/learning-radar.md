> **Supporting files:** this skill uses `template.html` shipped in the Cursor variant folder (`learning/cursor/learning-radar/`). Copy them next to wherever you keep this command's assets, or install the Cursor variant alongside and keep the default `~/.cursor/skills/learning-radar/` paths referenced in the text.
