> **Supporting files:** this skill uses `assets/template.html`, `feeds.toml`, `references/{workflow,gates,output}.md`, `scripts/scan_feeds.py`, `scripts/test_scan_feeds.py`, and `scripts/validate_triage.py` shipped in the Cursor variant folder (`learning/cursor/tech-radar/`). Copy them next to wherever you keep this command's assets, or install the Cursor variant alongside and keep the default `~/.cursor/skills/tech-radar/` paths referenced in the text.
