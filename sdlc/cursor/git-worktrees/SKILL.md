...
name: git-worktrees
description: >-
  Set up an isolated git worktree before feature work or plan execution when
  the current workspace must stay untouched (dirty tree, protected branch,
  parallel work). Detects existing isolation first, asks consent, creates
  .worktrees/<branch>, verifies setup. Use when the user asks for a worktree
  or execute-plan needs an isolated workspace.
---
