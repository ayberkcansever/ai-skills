...
5. Mark the todo `completed` only after its verification passes.
...
You *can* dispatch a fresh subagent per task for isolation or parallel attempts.
Only do this when the user explicitly asks for it, or when a task is large enough
that a clean, single-purpose context clearly helps. For most feature work, inline
is simpler.
